# Rubro gloves: posted-requirement hunt (2026-09-24)

## Prompt
Find buyers who have publicly posted a current requirement for rubber/latex/nitrile gloves
(household, industrial, examination, surgical), in India (Kerala/Kottayam first) or for export.
Sources: GeM, CPPP, Kerala e-tenders, KMSCL, hospital/PSU tenders, UNGM, B2B RFQs
(IndiaMART, TradeIndia, Go4WorldBusiness, Tradekey, Tradewheel), import data (Volza, Seair, ImportKey).
For each: buyer, quantity, deadline, contact route, cost. No generic directories.

## Found (ranked)
1. GeM tenders: live and recurring. Examples: sterile surgical gloves IS 13422 (Gorakhpur, closed 22 Sep 2026;
   AIIMS Rajkot, closed 9 Sep 2026); industrial safety gloves/gauntlets. Needs GeM seller registration + product listing.
   Aggregators: tender247, tender18 (1100+ glove bids), tendertiger, bidalert.
2. KMSCL (Kerala Medical Services Corp): 287 tenders in 2025; watch for glove items. Kerala govt buyer.
3. Export RFQs: go4WorldBusiness (glove/industrial RFQs, L/C terms), Tradekey (6 rubber-glove buy leads,
   23 working-glove, UK-heavy), Tradewheel (India + safety gloves). Membership needed to see contacts; verify every lead.
4. Import data (HS 401519): Volza lists 938 global buyers, 19,492 shipments, decision-maker contacts (paid).
   These are proven buyers (they already import), not posted requirements.

## Human steps
GeM registration (Udyam/GST/PAN), quoting on tenders, one paid B2B membership at most. Software never sends.

## Rubro product (from its website, 2026-09-24)
Natural-rubber gauntlet gloves, diamond-grip palm. All Purpose 10"-22" (115-305 g/pair),
Heavy Duty 10/12/14" HD (190-300 g/pair), Household 12-14" latex.
Uses: agriculture, industrial (oils, acids, alkalis), automotive, masonry/cement, household.
Not medical, not electrical-rated. Kozhuvanal, Kottayam 686584. Ph 6238778438.

## Matched targets
- Municipal/panchayat sanitation-supply tenders (gloves bundled with gum boots, phenyl, brooms) - tender247 lists.
- GeM "rubber hand gloves" / "industrial safety glove gauntlet" bids: PSUs, railways, KSEB, ports.
- Plantation/estate buyers (rubber, tea, coffee estates) - agriculture series.
- Export: Tradekey Saudi latex/household glove buyers (Durar Masagh Trading, Bassam Trading); go4WorldBusiness glove RFQs.
  Kenya, Egypt, South Africa are Africa's top glove importers. Verify every lead before quoting.

## Named buyers found (pass 2, per line)
- Sanitation/LSG: Koothuparamba Municipality tender - uniforms + PPE incl. gloves for Haritha Karma Sena. 58 Haritha Karma Sena tenders in 2026 (tenderdetail).
- Household: IndiaMART buy requests - Darshan Sheth (25 Apr 2026, household cleaning gloves); Seraj Siddiqui (long-term reusable household gloves, latex/rubber). Market price floor Rs14-16.50/pair.
- Export household/latex: Saudi - Durar Masagh Trading Est., Bassam Trading Co. (Tradekey RFQs).
- Plantation: PCK (8 estates, 6,452 ha) runs e-tenders (pcklimited.in/tender); no glove tender found - purchase dept is the route.
- Heavy duty/construction: no posted buyer found; retail only.

## Pass 3: named Kerala LSG glove tenders
- Koothuparamba Municipality: PPE incl. gloves, HKS + sanitation workers (listed 30 Apr 2026, kerala-tenders.co.in).
- Thirunelli (Wayanad): "Boot, Glouse, Uniform and other items to Haritha Karma Sena" (tendersontime).
- Thazhekode (Malappuram): Haritha Karma Sena uniform (tendersontime 112008085).
- Kasargod: uniforms, safety shoes, hand gloves, raincoats, PPE (tender247 81901848).
Pattern: every Kerala LSG buys HKS gloves yearly - ~1,000+ local bodies. Portal: etenders.kerala.gov.in (keyword "Haritha Karma Sena").
